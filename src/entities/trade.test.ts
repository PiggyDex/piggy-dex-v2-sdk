...
import { Ether, CurrencyAmount, Percent, Token, TradeType, WETH9, Price } from '@piggy-dex/sdk-core'

describe('Trade', () => {
  const ETHER = Ether.onChain(71)
  const token0 = new Token(71, '0x0000000000000000000000000000000000000001', 18, 't0')
  const token1 = new Token(71, '0x0000000000000000000000000000000000000002', 18, 't1')
  const token2 = new Token(71, '0x0000000000000000000000000000000000000003', 18, 't2')
  const token3 = new Token(71, '0x0000000000000000000000000000000000000004', 18, 't3')
  ...
  const pair_weth_0 = new Pair(
    CurrencyAmount.fromRawAmount(WETH9[71], JSBI.BigInt(1000)),
    CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(1000))
  )
  ...
  it('can be constructed with ETHER as input', () => {
    const trade = new Trade(
      ...
      CurrencyAmount.fromRawAmount(Ether.onChain(71), JSBI.BigInt(100)),
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for ETHER currency input', () => {
      ...
      expect(result[0].route.path).toEqual([WETH9[71], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(ETHER)
      expect(result[1].route.path).toEqual([WETH9[71], token0, token3])
    ...
    it('works for ETHER currency output', () => {
      ...
      expect(result[0].route.path).toEqual([token3, token0, WETH9[71]])
      expect(result[0].outputAmount.currency).toEqual(ETHER)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WETH9[71]])
